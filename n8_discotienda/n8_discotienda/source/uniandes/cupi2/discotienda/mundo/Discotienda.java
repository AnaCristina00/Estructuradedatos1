...
public class Discotienda
{
    ...
    /**
     * Generar un informe de todos los discos ROCK y POP, con un precio menor a 1000 pesos
     * @throws FileNotFoundException - cuando no existan discos con esas condiciones
     */
    public void generarInformeDiscosPR()throws FileNotFoundException
    {
    	//Creamos el archivo con la clase FILE
    	File archivoPR = new File("./data/discosCostosos.txt");
    	
    	//Crear la pluma para escribir el archivo
    	PrintWriter pluma = new PrintWriter(archivoPR);
    	
    	//Escribir con la pluma en el archivo
    	pluma.println("Reporte de discos de POP y ROCK");
    	pluma.println("===============================");
    	
    	//Escribir un reporte de discos con nombre, artista y género
    	boolean hayDiscosSeleccionados = false;
        	...
        	//Verificar si el genero es POP o ROCK y el precio es menor a mil pesos
        		
        	if((miDisco.darGenero().equalsIgnoreCase("Pop")|| miDisco.darGenero().equalsIgnoreCase("Rock"))
        		&& miDisco.darPrecioDisco()<=1000)
        	{
        		hayDiscosSeleccionados=true;
    ...
    if (!hayDiscosSeleccionados) {
        
        
    	// Lanzar una excepción si no hay discos que cumplan con los criterios
    	throw new IllegalArgumentException("No se encontraron discos que cumplan con los criterios");
    	    }
